...
impl XmlNode for MapType {
    fn start_bytes() -> BytesStart<'static> {
        BytesStart::new("MapType")
    }

    fn from_reader(reader: &mut Reader<&[u8]>) -> Option<Self> {
        let text = read_string(reader).unwrap_or_default();
        match text.as_str() {
            "Cyto" => Some(MapType::Cyto),
            "Bp" => Some(MapType::Bp),
            "CM" => Some(MapType::CM),
            "CR" => Some(MapType::CR),
            "Min" => Some(MapType::Min),
            _ => None,
        }
    }
}
...
impl XmlNode for MapsMethodChoice {
    fn start_bytes() -> BytesStart<'static> {
        BytesStart::new("MapsMethodChoice")
    }
    ...
    fn from_reader(reader: &mut Reader<&[u8]>) -> Option<Self> {
        loop {
            match reader.read_event().unwrap() {
                Event::Start(e) => match e.name().as_ref() {
                    b"Proxy" => return Some(MapsMethodChoice::Proxy(read_string(reader).unwrap_or_default())),
                    b"MapType" => {
                        if let Some(map_type) = MapType::from_reader(reader) {
                            return Some(MapsMethodChoice::MapType(map_type));
                        }
                    }
                    _ => (),
                },
...
impl XmlNode for Maps {
    ...
    fn from_reader(reader: &mut Reader<&[u8]>) -> Option<Self> {
        ...
        loop {
            match reader.read_event().unwrap() {
                Event::Start(e) => match e.name().as_ref() {
                    b"display-string" => maps.display_str = read_string(reader).unwrap_or_default(),
                    b"method" => maps.method = read_node(reader).unwrap_or(MapsMethodChoice::MapType(MapType::Bp)),
                    _ => (),
                },
                Event::End(e) => {
                    if e.name() == Self::start_bytes().name() {
...
impl XmlVecNode for GeneTrackStatus {}
