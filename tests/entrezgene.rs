...
use ncbi::entrezgene::{Entrezgene,EntrezgeneSet};

const DATA1: &str = "tp73.genbank.xml";

fn get_entrezgene(path: &str) -> Entrezgene {
    let set = get_seq_set(path);
    let entry = set.get(0).unwrap();
    print!("{:?}", entry);
/*
    match entry {
        Entrezgene(data) => return data.clone(),
        _ => panic!("Entry is not Entrezgene"),
    }
*/
    entry.clone()
}

fn get_seq_set(path: &str) -> EntrezgeneSet {
    let data = get_local_xml(path);
    let parsed = parse_xml(data.as_str()).unwrap();
    if let DataType::EntrezgeneSet(set) = parsed {
        return set;
    } else {
        panic!("No Entrezgene set found")
    }
}
